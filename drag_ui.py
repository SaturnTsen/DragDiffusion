...
with gr.Blocks() as demo:
    # a stable, single source of LENGTH (avoid creating ephemeral gr.Number components)
    length_state = gr.State(LENGTH)
        ...
        with gr.Row():
            with gr.Column():
                ...
                canvas = gr.Image(
                    ...
                    height=LENGTH,
                    width=LENGTH
                )  # for mask painting / image upload
        ...
        with gr.Tab("Base Model Config"):
            with gr.Row():
                ...
                vae_path = gr.Dropdown(
                    value="default",
                    label="VAE choice",
                    choices=["default", "stabilityai/sd-vae-ft-mse"] + local_models_choice
                )
    ...
    with gr.Tab(label="Editing Generated Image"):
        ...
        original_image_gen = gr.State(value=None)       # store generated image
        intermediate_latents_gen = gr.State(value=None) # store intermediate latents
        length_state_gen = gr.State(LENGTH)
        ...
        with gr.Tab("Generation Config"):
            with gr.Row():
                ...
                scheduler_name_gen = gr.Dropdown(
                    value="DDIM",
                    label="Scheduler",
                    choices=["DDIM", "DPM++2M", "DPM++2M_karras"]
                )
                n_inference_step_gen = gr.Number(value=50, label="Total Sampling Steps", precision=0)

        with gr.Tab("FreeU Parameters"):
            with gr.Row():
                b1_gen = gr.Slider(label="b1", info="1st stage backbone factor", minimum=1, maximum=1.6, step=0.05, value=1.0)
                b2_gen = gr.Slider(label="b2", info="2nd stage backbone factor", minimum=1, maximum=1.6, step=0.05, value=1.0)
                s1_gen = gr.Slider(label="s1", info="1st stage skip factor", minimum=0, maximum=1, step=0.05, value=1.0)
                s2_gen = gr.Slider(label="s2", info="2nd stage skip factor", minimum=0, maximum=1, step=0.05, value=1.0)
        ...
        with gr.Tab(label="Drag Config"):
            with gr.Row():
                ...
                inversion_strength_gen = gr.Slider(
                    0, 1.0,
                    value=0.7,
                    label="Inversion Strength",
                    info="The latent at [inversion-strength * total-sampling-steps] is optimized for dragging."
    ...
    #
    # IMPORTANT FIX:
    # - Users uploading/replacing an image may NOT trigger .edit()
    # - So we bind BOTH .upload() and .edit() to store_img
    #   so original_image State is always updated.
    canvas.upload(
        store_img,
        inputs=[canvas],
    ...
    clear_all_button.click(
        clear_all,
        inputs=[length_state],
        outputs=[canvas, input_image, output_image, selected_points, original_image, mask],
    )

    # Generated image:
    # Same fix: bind BOTH upload and edit (even though canvas_gen is interactive=False initially;
    # after generation it becomes interactive=True in your gen_img return)
    canvas_gen.upload(
        store_img_gen,
        inputs=[canvas_gen],
        outputs=[original_image_gen, selected_points_gen, input_image_gen, mask_gen],
    )
    canvas_gen.edit(
        store_img_gen,
        inputs=[canvas_gen],
    ...
    gen_img_button.click(
        ...
        outputs=[canvas_gen, input_image_gen, output_image_gen, mask_gen, intermediate_latents_gen],
    )

    undo_button_gen.click(
        undo_points,
        inputs=[original_image_gen, mask_gen],
        outputs=[input_image_gen, selected_points_gen],
    )
